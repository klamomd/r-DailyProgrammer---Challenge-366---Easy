USAGE: 
Run the project and a console window will show up. Type in either "funnel" or "bonus1" (without quotes) to choose which mode to run it in. In "funnel" mode, enter a big string and a little string to determine if the little string can be created by removing a single character from the big string. In "bonus1" mode, enter a big string to get a list of strings from wordList.txt that can be created by removing a single character.
...
for inspiring this week's challenges in r/dailyprogrammer_ideas!